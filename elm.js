...
Elm.Main.make = function (_elm) {
   ...
   var PickUpName = {ctor: "PickUpName"};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.input,_U.list([$Html$Attributes.value(model)]),_U.list([]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,PickUpName)]),_U.list([$Html.text("Pick up username")]))]));
   });
   var model = "Anonymous";
   var main = $StartApp$Simple.start({model: model,view: view,update: update});
   return _elm.Main.values = {_op: _op,main: main,model: model,view: view,PickUpName: PickUpName,update: update};
};
